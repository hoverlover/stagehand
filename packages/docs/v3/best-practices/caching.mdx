...
The GCS adapter uses Google Cloud Application Default Credentials (ADC) by default. You can also provide explicit credentials in several formats:
...
// Using a JSON string (common in serverless environments)
const adapter = new GCSAdapter({
  bucket: "my-stagehand-cache",
  credentials: process.env.GCS_CREDENTIALS, // JSON string
});

// Using a file path (for local development)
const adapter = new GCSAdapter({
  bucket: "my-stagehand-cache",
